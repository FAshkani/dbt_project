...
sql = '''
    create or replace temporary view dim_products__dbt_tmp as
-- CTE to rank CDC records by Id, meta_ExtractedDate, and SYS_CHANGE_VERSION
WITH source_data AS (
    select * FROM lh_silver.products t1
)
...
# MAGIC %%sql
# MAGIC     create or replace temporary view dim_products__dbt_tmp as
# MAGIC -- CTE to rank CDC records by Id, meta_ExtractedDate, and SYS_CHANGE_VERSION
# MAGIC WITH source_data AS (
# MAGIC     select * FROM lh_silver.products t1
# MAGIC )
...
# MAGIC   merge into lh_gold.dim_products as DBT_INTERNAL_DEST
# MAGIC       using dim_products__dbt_tmp as DBT_INTERNAL_SOURCE
# MAGIC       on 
# MAGIC               DBT_INTERNAL_SOURCE.ProductID = DBT_INTERNAL_DEST.ProductID
