...
sql = '''
    create or replace temporary view departments__dbt_tmp as
-- CTE to rank CDC records by Id, meta_ExtractedDate, and SYS_CHANGE_VERSION
WITH source_data AS (
    select * FROM lh_bronze.departments t1
)
...
# MAGIC %%sql
# MAGIC     create or replace temporary view departments__dbt_tmp as
# MAGIC -- CTE to rank CDC records by Id, meta_ExtractedDate, and SYS_CHANGE_VERSION
# MAGIC WITH source_data AS (
# MAGIC     select * FROM lh_bronze.departments t1
# MAGIC )
...
# MAGIC   merge into lh_silver.departments as DBT_INTERNAL_DEST
# MAGIC       using departments__dbt_tmp as DBT_INTERNAL_SOURCE
# MAGIC       on 
